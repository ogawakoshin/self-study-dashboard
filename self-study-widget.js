// Self-Study Dashboard -- Scriptable Widget
// Show today's tasks on the home screen

// ===== CONFIG =====
var WIDGET_URL = 'https://ogawakoshin.github.io/self-study-dashboard/widget.html';
...
var SCHEDULE = { 1:'takken', 2:'ap', 3:'takken', 4:'ap', 5:'denki', 6:'takken', 0:'ap' };
...
function getTakkenPlan(weekNum) {
  if (weekNum <= 2) {
    return [
...
function getTodayTasks() {
  ...
  var dayLabel = DAY_NAMES[dow];

  var currentPhase = 'takken';
  if (today >= new Date(2026, 10, 15)) {
    currentPhase = 'denki';
  } else if (today >= new Date(2026, 9, 19)) {
    currentPhase = 'ap';
  }

  var weekNum = Math.ceil(daysBetween(new Date(2026, 2, 26), today) / 7);

  var weekPlans = {
    takken: getTakkenPlan(weekNum),
    ap: [
  ...
  var plan = weekPlans[currentPhase] || [];
  return plan.filter(function(t) { return t.day === dayLabel; });
}
...
async function buildWidget() {
  ...
  var dateStr = months[today.getMonth()] + today.getDate() + '日(' + DAY_NAMES[dow] + ')';
  var dateText = header.addText(dateStr);
  dateText.font = Font.mediumSystemFont(11);
  dateText.textColor = new Color('#ffffff', 0.7);

  header.addSpacer();

  var countdownLabel = daysLeft > 0 ? exam.label + ' ' + daysLeft + '日' : exam.label + ' 終了';
  ...
  if (tasks.length > 0) {
    for (var i = 0; i < tasks.length; i++) {
      ...
      var timeText = taskRow.addText(task.time);
  ...
  var tapHint = footer.addText('タップで詳細を開く');
